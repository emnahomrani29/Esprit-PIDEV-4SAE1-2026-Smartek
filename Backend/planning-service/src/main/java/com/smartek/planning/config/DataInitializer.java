...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        // Corriger les sessions avec status null → DRAFT (requête SQL directe)
        int fixed = planningRepository.fixNullStatuses();
        if (fixed > 0) {
            System.out.println("Migration : " + fixed + " session(s) avec status null corrigée(s) → DRAFT");
        }
